...
class QLearningAgent(ReinforcementAgent):
  ...
  def getPolicy(self, state):
    ...
    maxAction = [action for action in legalActions if self.getQValue(state, action) == maxqValue]
    # random choose max action
  ...
  def update(self, state, action, nextState, reward):
    ...
    self.qVals[(state, action)] = (1.0 - self.alpha) * self.getQValue(state, action) + \
